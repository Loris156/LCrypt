...
namespace LCrypt.HashAlgorithms
{
    public class WhirlpoolManaged : HashAlgorithm
    {
        ...
        private const int DigestBits = 8 * DigestBytes;
        ...
        private readonly byte[] _bitLength = new byte[LengthBytes];
        private readonly byte[] _buffer = new byte[WBlockBytes];
        private readonly ulong[] _hash = new ulong[DigestBytes / 8];
        ...
        private static ulong Ll(ulong ul)
        {
            return ul;
        }
        ...
        private void NessieAdd(IReadOnlyList<byte> source, int sourcePosition, int sourceLength)
        {
            var sourceBits = (ulong) sourceLength * 8UL;
            var sourcePos = sourcePosition;
            var sourceGap = (8 - ((int) sourceBits & 7)) & 7;
            var bufferRem = _bufferBits & 7;
            int i;
            uint b, carry;
            var buffer = _buffer;
            var bitLength = _bitLength;
            var bufferBits = _bufferBits;
